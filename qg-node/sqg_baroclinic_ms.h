...
trace
void comp_stretch(scalar psi, scalar bs, scalar stretch, double add, double fac)
{
  ...
  foreach_vertex() {
    ...
    // upper layer
    point.l = 0;
    stretch[] = add*stretch[] + fac*(S2[]*bs[]*idh0[0] - S2[0,0,1]*(psi[] - psi[0,0,1])*idh1[point.l]);
    
    // intermediate layers
    for (point.l = 1; point.l < nl-1 ; point.l++) {
      stretch[] = add*stretch[] + fac*(S2[]*(psi[0,0,-1] - psi[])*idh0[point.l] - S2[0,0,1]*(psi[] - psi[0,0,1])*idh1[point.l]);
    }
    
    // lower layer
    point.l = nl-1;
    stretch[] = add*stretch[] + fac*(-S2[]*(psi[] - psi[0,0,-1]))*idh0[point.l];
    point.l = 0;
  }
...
trace
void rhs_pv_baroclinic(scalar q, scalar psi, scalar bs, scalar dqdt)
{
  ...
  foreach_vertex(){
    ...
    dqdt[] = -jacobian(psi, zeta) - jacobian(psi_pg, zeta) - S2[0,0,1]*jd*idh1[point.l]  \
      - beta_effect(psi);
      ...
      for (point.l = 1; point.l < nl-1 ; point.l++) {
        ...
        dqdt[] = -jacobian(psi, zeta) - jacobian(psi_pg, zeta) - S2[0,0,1]*jd*idh1[point.l] - S2[]*ju*idh0[point.l] \
          - beta_effect(psi);
      ...
      point.l = nl-1;
      ju = -jd;
      dqdt[] = -jacobian(psi, zeta) - jacobian(psi_pg, zeta) - S2[]*ju*idh0[point.l]  \
        - beta_effect(psi);
  ...
  vertex scalar del2_bs[];
  del2_bs[left]   = neumann(0.);
  del2_bs[right]  = neumann(0.);
  del2_bs[bottom] = neumann(0.);
  del2_bs[top]    = neumann(0.);

  foreach_vertex()
      del2_bs[] = laplacian(bs);
  ...
  comp_stretch(zeta, del2_bs, dqdt, 1., nu);
//  comp_del2(zeta, dqdt, 1., nu);
  ...
  vertex scalar del4_bs[];
  del4_bs[left]   = neumann(0.);
  del4_bs[right]  = neumann(0.);
  del4_bs[bottom] = neumann(0.);
  del4_bs[top]    = neumann(0.);

  foreach_vertex()
      del4_bs[] = laplacian(bs);
  ...
  comp_stretch(tmp, del4_bs, dqdt, 1., minus_nu4);
...
// TODO: STOPPED HERE
trace
void rhs_bs(scalar bs, scalar psi, scalar dbdt)
{
...
// TODO mask for b
  ...
  foreach_vertex() {
    ...
    // todo: neumann BC for bs
    double pt_weight = mask[1] + mask[-1] + mask[0,1] + mask[0,-1] \
      + mask[1,1] + mask[-1,-1] + mask[-1,1] + mask[1,-1];
      
    if (mask[] == 0 && (pt_weight !=0))
      bs[] = 
  }
}
...
static void relax_baroclinic (scalar * al, scalar * bl, int l, void * data)
{
  ...
  foreach_vertex_level(l) {
    ...
    t2[point.l] = -sq(Delta)*S2[0,0,1]*idh1[point.l]*mask[];
    ...
    for (point.l = 1; point.l < nl-1 ; point.l++) {
      rhs[point.l] = - sq(Delta)*b[]*mask[];
      t0[point.l] = -sq(Delta)*S2[]*idh0[point.l]*mask[];
      t2[point.l] = -sq(Delta)*S2[0,0,1]*idh1[point.l]*mask[];
    ...
    rhs[point.l] = - sq(Delta)*b[]*mask[];
    t0[point.l] = -sq(Delta)*S2[]*idh0[point.l];
...
static double residual_baroclinic (scalar * al, scalar * bl, scalar * resl, void * data)
{
  ...
  double maxres = 0.;
    ...
    point.l = 0;
      ...
      res[] = (b[] + S2[0,0,1]*(a[] - a[0,0,1])*idh1[point.l])*mask[];
      foreach_dimension() {
    ...
    // intermediate layers
    for (point.l = 1; point.l < nl-1 ; point.l++) {
      res[] = (b[] + S2[]*(a[] - a[0,0,-1])*idh0[point.l] - S2[0,0,1]*(a[0,0,1] - a[])*idh1[point.l])*mask[];
      foreach_dimension() {
          res[] -= (a[-1] - 2.*a[] + a[1])/(sq(Delta))*mask[];
      }
      if (fabs(res[]) > maxres)
    ...
    res[] = (b[] + S2[]*(a[] - a[0,0,-1])*idh0[point.l])*mask[];
      foreach_dimension() {
          res[] -= (a[-1] - 2.*a[] + a[1])/(sq(Delta))*mask[];
      }
      if (fabs(res[]) > maxres)
...
event init (i = 0){
  ...
  idh0[0] = 1./dh[0]; // surface layer: 1/h
  idh1[0] = 1./(dhc[0]*dh[0]);
  ...
  if ((fp = fopen(name, "r"))) {
    ...
    char * N2_sav = S2.name; 
    S2.name = N2_name;
    read_nc({S2, psi_pg, mask, topo}, name, false);
//    read_nc({S2, psi_pg}, name);
  ...
  foreach_vertex(){
    S2[] = f_var/S2[]; // surface layer f^2/N^2
    for (point.l = 1; point.l < nl ; point.l++)
      S2[] = sq(f_var)/S2[]; // interior layer: f^2/N^2
    point.l = 0;
  }
  ...
  for (int l = depth(); l >= 0; l--) {
    foreach_vertex_level (l) {
      ...
      if (ref_flag > 0)
        ...
        double L_filt2 = 0;
        if (fac_filt_Rd > 0){
          L_filt2 =  min(fac_filt_Rd*dh[0]/sqrt(S2[0,0,1]),Lfmax);
        }
        else
          L_filt2 = L_filt;
